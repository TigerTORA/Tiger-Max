...
class DoIt(object):
    ...
    def cmd_ctrl(self):
        ...
        stout = stdout.read()
        sterr = stderr.read()
        # self._output("&^&"+str([self.ip,stdout.read(),stderr.read()])+"@$@")
        #self._output(str([stdout.read(),stderr.read()]))
        ssh_conn.close()
        return [self.ip, stout, sterr]
...
class WorkManager(object):
    ...
    def __init_thread_pool(self):
        ...
        for i in range(self.thread_pool_size):
            self.threads_list.append(Work(self.work_queue, self.res_queue, self.base))
    ...
    def wait_all_complete(self):
        ...
        while not self.res_queue.empty():
            result_list.append(self.res_queue.get())
        return result_list
...
class Work(Thread):
    ...
    def __init__(self, work_queue, res_queue,base_object):
...
def hhdp(args):
    ...
    return work_manager.wait_all_complete()
