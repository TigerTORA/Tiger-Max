...
class Process(object):
    ...
    @staticmethod
    def exists_hhdp(program):
        """
        使用hhdp工具在所有的主机上判断是否安装program
        """
        unexists_list = []
        hostnm = "echo -n $(hostname)###"
        (out, err) = Process.call("hhdp -c '%s&&which %s'" %(hostnm,program))

        out_list = out.splitlines(True)
        err_list = err.split('/n')
        #print(len(out_list))
        for line in out_list:
            ...
            key = key_value[0]
            value = key_value[1]
            if "which: no" in value:
                unexists_list.append(key)
